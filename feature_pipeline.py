...
def get_weather_df(api_key, city, index=0):
    ...
    df['precipprob'] = df["precipprob"].astype("float32").round().astype("uint32")
    df['precipcover'] = df["precipcover"].astype("float64")
    df['snow'] = df["snow"].astype("float32").round().astype("uint32")
    df['snowdepth'] = df["snowdepth"].astype("float32").round().astype("uint32")
    ...
    df['uvindex'] = df["uvindex"].astype("float32").round().astype("uint32")
